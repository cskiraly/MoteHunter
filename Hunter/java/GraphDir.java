...
/* Panel for drawing mote-data GraphDirDirs */
class GraphDir extends JPanel
{
  ...
  protected void paintComponent (Graphics g)
  {
    ...
    synchronized (parent.parent) {
      ...
      for (int i = 0; i < count; i++) {
        if (parent.moteListModel.getShowYaw (i)) {
  ...
  {
    SingleGraph sg = new SingleGraph (g, nodeId, gx0, gx1, 90);
    sg.plot (g, color);
  }
  ...
  private class SingleGraph
  {
      ...
      SingleGraph (Graphics g, int id, int from, int to, int bins)
    {
      ...
      gx1 = to;
      this.bins = bins;
      deg2power = new double[bins];
        deg2count = new double[bins];
        deg2age = new double[bins];
      ...
      for (int gx = gx0; gx <= gx1; gx++) {
        ...
        if (power >= 0 && yaw >= 0) {
          if (nodeId == 998 || nodeId == 999) {
            deg2power[(int) ((yaw / 360) * bins)] =
              Math.max (power, deg2power[(int) ((yaw / 360) * bins)]);
            deg2count[(int) ((yaw / 360) * bins)] = 1;
          } else {
            double weight = (double) (gx - gx0) / (gx1 - gx0);
            deg2power[(int) ((yaw / 360) * bins)] += power * weight;
            deg2count[(int) ((yaw / 360) * bins)] += weight;
          }
          deg2age[(int) ((yaw / 360) * bins)] = (double) (gx - gx0) / (gx1 - gx0);
        }
      }
    }
    ...
    void plot (Graphics g, Color color)
    {
      for (int x = 0; x < bins; x++) {
        int sx;
        if (compass != null) {
          sx = ((x + bins + bins / 2 - compass.getYaw () * bins / 360) % bins) * width / bins;
        } else {
          sx = x * width / bins;
        }

        int sy = (deg2count[x] > 0) ? screenY (deg2power[x] / deg2count[x]) : -1;

        if (sy >= 0) {
          g.setColor (fadeColor (color, deg2age[x]));
          g.drawOval (sx, sy, 3, 3);
          if (lastsy >= 0 && sx != 0) {
