...
class Node
{
  /* Data is hold in an array whose size is a multiple of INCREMENT. */
    ...
    if (newEnd > dataStart + data.length) {
      ...
      if (newEnd > dataStart + data.length) {
        ...
        if (dataStart + data.length > newStart) {
          ...
          System.arraycopy (count, newStart - dataStart, count, 0,
                            count.length - (newStart - dataStart));
        }
        dataStart = newStart;
      }
    }
  ...
  /* Data received containing a sample from messageId onwards */
  void update (int messageId, int reading)
  {
    int start = messageId;
    setEnd (start, start + 1);
    data[start - dataStart] += reading;
    count[start - dataStart]++;
  }
  ...
  double getData (int x)
  {
    if (x < dataStart || x >= dataEnd) {
      return -1;
    } else if (count[x - dataStart] == 0) {
      return -1;
    } else {
      return ((double) data[x - dataStart]) / count[x - dataStart];
    }
  }
